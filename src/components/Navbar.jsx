...
const Navbar = () => {
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 pt-5 sm:pt-8 ${scrolled ? 'backdrop-blur-md bg-white/40' : ''}`}>
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-12">
          <div className="flex items-center justify-between">
            {/* Logo */}
            <a href="#home" className="flex items-center z-50 transition-transform duration-300 hover:scale-105">
            ...
            <button
              ...
              className="lg:hidden relative z-50 flex items-center gap-2.5 bg-white px-5 py-2.5 rounded-full shadow-[0_4px_20px_-4px_rgba(0,0,0,0.08)] border border-white text-[#0F172A] font-semibold text-[13px] hover:bg-gray-50 transition-colors"
        ...
        <div className="flex flex-col pt-28 px-8 gap-2 flex-1">
          {navLinks.map((link, index) => (
            <a
              ...
              className={`px-5 py-3.5 rounded-2xl font-semibold text-base transition-all ${activeIndex === index ? 'bg-[#F4F7FF] text-[#2563EB]' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}
            >
              {link.name}
            </a>
          ))}
